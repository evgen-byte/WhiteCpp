...
//часто принято называть заглавными буквами
...
int main(void) {
	...
	for (int i = 0; i < q; ++i) {
		...
		if (operation_code == "ADD") {
			...
			//переходим к следующему месяцу запомним длину предыдущего
			//объявляем эту переменную константной, потому что менять её не планируем
			const int old_month_length = MONTH_LENGTHS[month];

		//номер месяца должен увеличиться на 1, но после декабря идет январь:
	    //например, (5 + 1) % 12 = 6, но (11 + 1) % 12 = 0
			...
			if (new_month_length < old_month_length) {
				...
				//далее понадобится добавлять новые дела в последний день нового месца
			...
			for (const string& s : days_concerns[day]) {
				cout << s << " ";
			}
			cout << endl;
		}
